...
class FirebaseService {
  ...
  Future<void> saveVolunteerData(String uid, Map<String, dynamic> volunteerData) async {
    try {
  ...
  Future<void> updateVolunteerEmail(String uid, String newEmail) async {
    try {
      await _firestore.collection('volunteers').doc(uid).update({'email': newEmail});
    } catch (e) {
  ...
  Future<void> saveProfileData(Map<String, dynamic> profileData) async {
    try {
      ...
      await _firestore.collection(AppConstants.volunteerProfilesCollection).doc(uid).set(profileData);
    } catch (e) {
